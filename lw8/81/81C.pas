        ...
        THEN
          BEGIN
            ...
            DO { По крайней мере два символа остается для Ch1,Ch2 }
              BEGIN
                READ(F1, Ch2);
                IF Ch1 <= Ch2 { Выводим   min(Ch1,Ch2) в F2, записывая отсортированные символы }
                THEN
                  BEGIN
                    WRITE(F2, Ch1);
                    WRITE(Ch1); {Отладочный оператор. Выводит на экран содержимое Ch1}
                    Ch1 := Ch2
                  END
                ELSE
                  BEGIN
                    WRITE(F2, Ch2);
                    WRITE(Ch2); {Отладочный оператор. Выводит на экран содержимое Ch2}
                    Sorted := 'N'
                  END
              END;
            WRITELN(F2, Ch1); { Выводим последний символ в F2 }
            WRITE(Ch1); {Отладочный оператор. Выводит на экран содержимое Сh1}
          END;
      END;
      { Копируем F2 в F1 }
