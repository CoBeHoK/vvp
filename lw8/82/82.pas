  ...
  BEGIN { Копируем INPUT в F1 }
    ...
    DO
      BEGIN
        READ(Ch);
        WRITE(F1, Ch);
        WRITE(Ch) {Отладочный оператор вывода содержимого файла F1}
      END; {Отладочный оператор вывода содержимого файла F1}
    WRITELN
  END;
  Sorted := 'N';
  WHILE Sorted = 'N'
      ...
      BEGIN { Копируем F2 в F1 }
        ...
        DO
          BEGIN
            ...
            WRITE(Ch); {Отладочный оператор вывода содержимого файла F1}
          END;
        WRITELN; {Отладочный оператор вывода содержимого файла F1}
        WRITELN(F1)
      END
    END;
